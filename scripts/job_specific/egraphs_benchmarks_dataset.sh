#!/bin/bash
#
# Job-specific script for egraphs benchmarks dataset generation
# This script contains the core logic for the egraphs benchmarks dataset job
#
...
# Job configuration
JOB_NAME="egraphs_benchmarks_dataset"
PROJECT_NAME="egraphs_benchmarks"

# Default configuration (can be overridden by config file or environment variables)
: ${PROJECT_DIR:="/scratch/$USER/chehab/egraphs"}
: ${INPUT_FILE:="/scratch/$USER/chehab/RL/fhe_rl/datasets/benchmarks.txt"}
: ${OUTPUT_FILE:="benchmarks_traces.json"}
: ${DATASET_LIMIT:=10000}
